...
#include <social_octomap_ros/SocialOctomapROS.h>
...
int main(int argc, char **argv)
{
  ...
  geometry_msgs::Point originPt = pointSocialOctomapToMsg(origin);
  ...
  for (int i = 0; i < 360; i++)
  {
    for (int j = 0; j < 360; j++)
    {
      geometry_msgs::Point endPt = pointSocialOctomapToMsg(origin + point_on_surface);
  ...
  for (int i = 0; i < 360; i++)
  {
    for (int j = 0; j < 360; j++)
    {
      geometry_msgs::Point directionPt = pointSocialOctomapToMsg(direction);
      ...
      {
        hit++;
        point3d obstacle = pointMsgToSocialOctomap(obstaclePt);
  ...
  geometry_msgs::Point single_originPt = pointSocialOctomapToMsg(single_origin);
  point3d single_end(beamLength, 0.0, 0.0);

  for (int i = 0; i < num_beams; i++)
  {
    geometry_msgs::Point single_endPt = pointSocialOctomapToMsg(single_origin + single_end);
